...
export default function Admin() {
  ...
  async function fetchPaidOrders() {
    ...
    while (attempts < maxRetries) {
      try {
        ...
        if (!response.ok) throw new Error(`HTTP ${response.status}`);
        const data = await response.json();
        console.log(`Paid orders fetched successfully: ${data.length} orders`);
  ...
  const getAnalytics = () => {
    ...
    const totalRevenue = todaysOrders.reduce((sum, order) =>
      sum + order.items.reduce((s, item) => s + (item.price * (item.quantity || 1)), 0), 0) || 0;
    const itemCounts = {};
    ...
    const totalItemsSold = todaysOrders.reduce((sum, order) =>
      sum + order.items.reduce((s, item) => s + (item.quantity || 1), 0), 0) || 0;
    const aov = totalOrders > 0 ? totalRevenue / totalOrders : 0;
    return { totalOrders, totalRevenue, mostSoldItem, peakHour: peakHour === -1 ? 'N/A' : peakHour, totalItemsSold, aov };
  };
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                  ...
                  <div className="bg-gradient-to-r from-orange-500 to-orange-600 text-white p-6 rounded-lg shadow-lg transform hover:scale-105 transition">
                    ...
                    <p className="text-2xl font-bold">
                      {analytics.peakHour === 'N/A' ? 'N/A' : `${analytics.peakHour}:00`}
                    </p>
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
