...
export default function Waiter() {
  ...
  const { data: ordersData, error: ordersError, isLoading: isOrdersLoading } = useSWR(
  ...
  // Debug menu and editing state
  useEffect(() => {
    console.log('Waiter state:', { menuLength: menu?.length, editingOrder, isCartOpen, isEditing: !!editingOrder });
  }, [menu, editingOrder, isCartOpen]);
  ...
  const saveEditedOrder = async () => {
    ...
    try {
      setError(null);
      console.log('saveEditedOrder payload:', { items: cart, notes: orderNote });
